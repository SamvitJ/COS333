...
4) Merging decisions must ahere to a variant of the gitflow branching model (nvie.com/posts/a-successful-git-branching-model) described here:

- A permanent master branch holds stable, in-production code, with tagged commits representing major release versions. 

- A permanent develop branch holds a work-in-progress version of the product; changes made here are integration-tested in the staging environment before being merged into master.
...
"The seven rules of a great git commit message" (chris.beams.io/posts/git-commit).
